...
public sealed class PlaceNameExtractor : BaseSheetExtractor
{
    public override string SheetName => "PlaceName";
    public override string DefaultJsonFileName => "placename.json";
    public override string Description => "Nomi di luoghi, territori, città e zone del gioco.";

    public override int ExtractAndSave(GameData lumina, string targetJsonPath)
    {
        var exd = lumina.GetFile("exd/placename_0_en.exd");
        if (exd == null) throw new InvalidOperationException("exd/placename_0_en.exd non trovato.");
